...
sub valgrind {
	...
	$_toolName = "Valgrind";
	my $template = "$myDirectory/tools/valgrind.sh $_testName \"-I$myDirectory/../tests/juliet/testcasesupport %s -DINCLUDEMAIN -gdwarf-2 -lm -x c -O0 -m32 -U __GNUC__ -pedantic -std=c99 $myDirectory/../tests/juliet/testcasesupport/io.c $file\" ";
...
sub ubc {
	my ($file) = (@_);
	# tools/ubc.sh CWE680_Integer_Overflow_To_Buffer_Overflow__malloc_fixed_01.c "-I../tests/juliet/testcasesupport -DOMITGOOD -DINCLUDEMAIN -lm -x c -O0 -m32 -U __GNUC__ -pedantic -std=c99 ../tests/juliet/testcasesupport/io.c CWE680_Integer_Overflow_To_Buffer_Overflow__malloc_fixed_01.c"
	$_toolName = "ubc";
...
sub kcc {
	...
	$_toolName = "kcc";
	my $template = "$myDirectory/tools/kcc.sh $_testName \"-I$myDirectory/../tests/juliet/testcasesupport -DINCLUDEMAIN %s $myDirectory/../tests/juliet/testcasesupport/io.c $file\"";
	my $goodCommand = $template;
	$goodCommand =~ s/%s/-DOMITBAD/;
	my $badCommand = $template;
	$badCommand =~ s/%s/-DOMITGOOD/;
...
sub ccured {
	...
	$_toolName = "ccured";
	my $template = "$myDirectory/tools/ccured.sh $_testName \"--includedir=$myDirectory/../tests/juliet/testcasesupport -DINCLUDEMAIN %s $myDirectory/../tests/juliet/testcasesupport/io.c $file\"";
	my $goodCommand = $template;
	$goodCommand =~ s/%s/-DOMITBAD/;
	my $badCommand = $template;
	$badCommand =~ s/%s/-DOMITGOOD/;
...
sub checkPointer {
	...
	my $template = "$myDirectory/tools/checkpointer.sh $_testName \"$file\" \"-I../../tests/juliet/testcasesupport\" \"$secfiles\" \"%s\"";
	my $goodCommand = $template;
	$goodCommand =~ s/%s/-config ..\/initGood.h/;
	my $badCommand = $template;
	$badCommand =~ s/%s/-config ..\/initBad.h/;
	genericTestHandler($goodCommand, $badCommand);
}
