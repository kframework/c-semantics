...
$dbh->do("CREATE TABLE if not exists rules (	
	...
	, type NOT NULL
	, kind NOT NULL
	, PRIMARY KEY (locationFile, locationFrom, locationTo)
)");
...
while(<MYINPUTFILE>) {
	...
	if ($line =~ m/^\s*(?:eq|ceq|rl|crl) .*\[.*metadata.*(structural|computational).*\]\.\s*$/){
		$kind = $1;
	} else {
		$kind = 'macro';
	}
	if ($locationFile && $locationFrom && $locationTo) { 
		$insertHandle->execute($ruleName, $locationFile, $locationFrom, $locationTo, $isLibrary, $type, $kind);
	}
}
