...
use RV_Kcc::Files qw(profileDir distDir tempFile tempDir error IS_CYGWIN);
use RV_Kcc::Opts qw(
...
sub main {
      ...
      if (arg('-frunner-script') || !$useCompile) {
            ...
            my $sem = profileDir('c11-cpp14-kompiled', 'c11-cpp14-kompiled');
            my $d = tempDir('native');
            ...
            if (arg('-fnative-binary')) {
                  ...
                  shell('gcc', '-std=c11', '-o', catfile($d, 'native_binary'),
...
sub makeOptions {
      ...
      for (breakpoints()) {
            my ($file, $line) = @{$_};
            $file = quote(backslash(quote(rel2abs($file))));
            $line = quote(backslash($line));
...
sub getLinkingCommand {
      ...
      my @krun = getKRunCommand($output, $symbols, 'c11-cpp14-linking-kompiled');
...
sub getTranslationCommand {
      ...
      my @krun = getKRunCommand($output, 'dummy',
            $lang eq 'c++'? 'cpp14-translation-kompiled' : 'c11-translation-kompiled');
...
sub dumpSymbols {
      state $syms = do {
            ...
            shell(distDir('gccsymdump'), profileDir('native', 'builtins.o'), ldArgs())->stdout($tmp)->run();
            $tmp;
      };
      return $syms;
}
...
sub getStd {
      state $c45 = ! shell('gcc -std=c45 -E /dev/null')->result(); # sanity check
      state $cpp14 = ! shell('g++ -std=c++14 -E /dev/null')->result();
      state $c11 = ! shell('gcc -std=c11 -E /dev/null')->result();
