...
let rec fold_right1 (f : 'a -> 'a -> 'a) : 'a list -> 'a = function
  | [x]     -> x
...
let sequence : (unit printer) list -> unit printer = fold_left1 (fun a b -> (a >> puts ", " >> b))
...
and translation_unit (filename : string) (defs : definition list) (s : printer_state) =
...
and specifier_elem sort = function
  ...
  | SpecPattern name -> kapply SpecifierElem sort "SpecPattern" [identifier CId name]
and specifier sort a = kapply Specifier sort "Specifier" [list_of specifier_elem KItem a]
and definition (sort : csort) : definition -> unit printer = function
...
and definition_loc sort a l = if l.lineno <> -10 then kapply KItem sort "DefinitionLoc" [a; cabs_loc CabsLoc l] else a
and definition_loc_range sort a b c = kapply KItem sort "DefinitionLocRange" [a; cabs_loc CabsLoc b; cabs_loc CabsLoc c]
...
and init_what sort = function
  ...
  | ATINDEXRANGE_INIT (exp1, exp2) -> kapply KResult sort "AtIndexRangeInit" [expression KItem exp1; expression KItem exp2]
and init_fragment sort (a, b) = kapply KItem sort "InitFragment" [init_what KItem a; init_expression KItem b]
...
and init_name sort (a, b) = kapply KItem sort "InitName" [name KItem a; init_expression K b]
and init_name_group sort (a, b) = kapply KItem sort "InitNameGroup" [specifier KItem a; list_of init_name KItem b]
and name_group sort (a, b) = kapply KItem sort "NameGroup" [specifier KItem a; list_of name KItem b]
and field sort (n, expOpt) = match expOpt with
  | None     -> kapply KItem sort "FieldName" [name KItem n]
  | Some exp -> kapply KItem sort "BitFieldName" [name KItem n; expression KItem exp]
and field_group sort (spec, fields) = kapply KItem sort "FieldGroup" [specifier KItem spec; list_of field KItem fields]
...
and float_literal sort r =
  let hex_float_constant f =
    ...
    kapply FloatConstant sort "HexFloatConstant" [significandPart; exponentPart; approxPart] in
  let dec_float_constant f =
    ...
    kapply FloatConstant sort "DecimalFloatConstant" [significandPart; exponentPart; approxPart] in
    ...
    let firstTwo = if String.length r > 2 then Str.first_chars r 2 else "xx" in
      if firstTwo = "0x" or firstTwo = "0X" then
        let nonPrefix = Str.string_after r 2 in
          hex_float_constant nonPrefix
      else dec_float_constant r in
  match tag with
    | ["F"] -> kapply Constant sort "LitF" [num Constant]
    | ["L"] -> kapply Constant sort "LitL" [num Constant]
    | []    -> kapply Constant sort "NoSuffix" [num Constant]
and int_literal sort i =
  let hex_constant (i : string) = kapply IntConstant sort "HexConstant" [ktoken_string i] in
  let oct_constant (i : string) = kapply IntConstant sort "OctalConstant" [ktoken_int_of_string i] in
  let dec_constant (i : string) = kapply IntConstant sort "DecimalConstant" [ktoken_int_of_string i] in
  ...
  let num sort = (
    ...
    let firstOne = if String.length i > 1 then Str.first_chars i 1 else "x" in
      if firstTwo = "0x" or firstTwo = "0X" then hex_constant (Str.string_after i 2)
      else (if firstOne = "0" then oct_constant (Str.string_after i 1) else (dec_constant i))) in
  match tag with
    | ["U"; "L"; "L"] | ["L"; "L"; "U"] -> kapply1 Constant sort "LitULL" (num Constant)
    | ["L"; "L"]                        -> kapply1 Constant sort "LitLL" (num Constant)
    | ["U"; "L"] | ["L"; "U"]           -> kapply1 Constant sort "LitUL" (num Constant)
    | ["U"]                             -> kapply1 Constant sort "LitU" (num Constant)
    | ["L"]                             -> kapply1 Constant sort "LitL" (num Constant)
    | []                                -> kapply1 Constant sort "NoSuffix" (num Constant)

and expression sort = function
  ...
  | GENERIC (exp, assocs)                                      -> kapply KItem sort "Generic" [expression K exp; list_of printGenericAssoc KItem assocs]
...
and printGenericAssoc sort = function
  ...
  | GENERIC_DEFAULT exp                  -> kapply KItem sort "GenericDefault" [expression KItem exp]
and unary_expression sort op exp =
  ...
  kapply KItem sort (unary_operator op) [expression KItem exp]
and binary_expression sort op exp1 exp2 =
  ...
  kapply KItem sort (binary_operator op) [expression KItem exp1; expression KItem exp2]
and block_statement sort blk = kapply KItem sort "BlockStatement" [block KItem blk]
and new_block_statement sort s = block_statement sort { blabels = []; battrs = []; bstmts = [s] }
and statement sort =
